export const usePropertyFormat = (property) => {
   ...
   const amenities = property.amenities? property.amenities.flatMap(( {amenities} ) => amenities).map((item) =>  (item)) :'' ;//(property.amenities.length > 0) property.amenities.flatMap(( {amenities} ) => amenities).map((item) =>  item.text ) : '' ;
